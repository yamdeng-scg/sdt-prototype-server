...
// 중복 계정 찾기
router.post('/accountAlreadyCheck',
    function (req, res, next) {
        // validate : id
        const id = req.body.id;
        dbService.selectQueryById('findUserByLoginId', [id])
            .then((result) => {
                if (result.length > 0) {
...
router.post('/register',
    function (req, res, next) {
        ...
        dbService.insert('gateway_user', registerUser)
            .then(() => {
                res.send({ success: true });
                dbService.insert('table_change_info', { table_name: 'gateway_user', column_name: 'all' });
            }).catch(errorRouteHandler(next));
    });
            ...
            .then((result) => {
                if (result.length > 0) {
                    ...
                    if (passwordHash.verify(password, findUser.password)) {
                        let authJsonWebToken = jwt.sign(Object.assign({}, findUser), CONFIG.JSONTOKEN_SECRETKEY, { expiresIn: CONFIG.JSONTOKEN_EXPIRE });
...
router.post('/searchPassword',
    function (req, res, next) {
        ...
        dbService.selectQueryById('findUserByEmail', [email])
            .then((result) => {
                if (result.length > 0) {
                    // let findUser = result[0];
                    // let template = {
                    ...
                    return Promise.reject(new AppError('존재하지 않는 계정정보입니다. 이메일 주소를 확인해주세요'));
                }
            }).then(() => {
                res.send({ success: true });
            }).catch(errorRouteHandler(next));
...
router.get('/loginUserInfo', function (req, res) {
    let loginUser = null;
    try {
        loginUser = jwt.verify(req.headers.authorization, CONFIG.JSONTOKEN_SECRETKEY);
    } catch (err) {
    ...
    try {
        ...
        if (ifconfigChild && ifconfigChild.stdout && ifconfigChild.toString) {
            let networkInterfaceInfoArray = ifconfigChild.stdout.toString().split('\n');
            if (networkInterfaceInfoArray && networkInterfaceInfoArray.length) {
                for (let arrayIndex = 0; arrayIndex < networkInterfaceInfoArray.length; arrayIndex++) {
...
// 관리자계정 정보 수정
router.post('/changeAdminPassword', authMiddleware,
    function (req, res, next) {
        // validate : 'oriPassword', 'newPassword'
        const oriPassword = req.body.oriPassword;
        const newPassword = req.body.newPassword;
        dbService.selectQueryById('findUserByLoginId', [CONFIG.ADMIN_INFO.id])
            .then((result) => {
                if (result.length > 0) {
                    let findUser = result[0];
                    if (passwordHash.verify(oriPassword, findUser.password)) {
                        let encodedPassword = passwordHash.generate(newPassword);
                        return dbService.update('gateway_user', { password: encodedPassword }, 'id', CONFIG.ADMIN_INFO.id);
                    } else {
                        return Promise.reject(new AppError('비밀번호가 맞지않습니다'));
                    }
                } else {
