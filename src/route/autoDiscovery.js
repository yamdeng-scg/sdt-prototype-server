...
router.get('/superAll', authMiddleware, function (req, res, next) {
  ...
  if (serverType === 'L') {
    promiseList.push(
      dbService.selectQueryById('treeDisplayGatewayList').then((result) => {
  ...
  promiseList.push(
    dbService.selectQueryById('treeDisplayCh1DeviceList').then((result) => {
      result.forEach((info) => {
        info.device_kind = deviceTypeMapping.getDeviceKind(
          info.device_type,
          info.product_code
        );
      });
  ...
  promiseList.push(
    dbService.selectQueryById('treeDisplayCh2DeviceList').then((result) => {
      result.forEach((info) => {
        info.device_kind = deviceTypeMapping.getDeviceKind(
          info.device_type,
          info.product_code
  ...
  promiseList.push(
    dbService.select('gateway_data').then((result) => {
  ...
  promiseList.push(
    dbService.selectQueryById('discoveredGateways').then((result) => {
  ...
  promiseList.push(
    dbService.selectQueryById('ch1DiscoveredDevices').then((result) => {
      if (result && result.length > 0) {
  ...
  promiseList.push(
    dbService.selectQueryById('ch2DiscoveredDevices').then((result) => {
      if (result && result.length > 0) {
    ...
    dbService.select('setting_info').then((result) => {
      result.forEach((info) => {
        ...
        if (
          ...
          cloudResponse.status !== STATIC_CONFIG.CLOUD_STATUS_ING
        ) {
    ...
    .then(() => {
      ...
      if (serverType === 'L') {
        gatewayTreeList.forEach(function (info) {
          let token = new Buffer(moment().format('YYYYMMDD') + 'lsis').toString(
            'base64'
          );
          info.link_path =
            'http://' +
            ...
            '/api/application/manualLogin?token=' +
            token;
