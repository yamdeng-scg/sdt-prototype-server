      ...
      dbService
        ...
        .then(() => {
          dbService.insert('table_change_info', {
            ...
            column_name: 'ch1_discovery_start'
          });
        });
    })
...
router.post('/autoDiscoveryStop', function (req, res, next) {
  dbService
    ...
    .then(() => {
      ...
      dbService.insert('table_change_info', {
        table_name: 'control_discovery',
        column_name: 'ch1_discovery_stop'
      });
    })
    .catch(errorRouteHandler(next));
});
  ...
  dbService
    ...
    .then(() => {
      dbService.executeQueryByStr(
        'delete from info_event_log_ch1 where channel_number = ?',
        [1]
      );
      dbService.executeQueryByStr(
        'delete from fault_event_log_Ch1 where channel_number = ?',
        [1]
      );
      dbService.executeQueryByStr(
        'delete from tripwave_log_ch1 where channel_number = ?',
        [1]
      );
      dbService.executeQueryByStr(
...
router.get('/backupDiscoveredDevices', function (req, res, next) {
  dbService
    .selectQueryById('ch1BackupDiscoveredDevices')
    .then((result) => {
      res.send(result);
    })
    .catch(errorRouteHandler(next));
...
router.get('/discoveredDevices', function (req, res, next) {
  dbService
    .selectQueryById('ch1DiscoveredDevices')
    .then((result) => {
      res.send(result);
    })
    .catch(errorRouteHandler(next));
