      ...
      dbService
        ...
        .then(() => {
          dbService.insert('table_change_info', {
            ...
            column_name: 'ch2_discovery_start'
          });
        });
    })
...
router.post('/autoDiscoveryStop', function (req, res, next) {
  dbService
    ...
    .then(() => {
      ...
      dbService.insert('table_change_info', {
        table_name: 'control_discovery',
        column_name: 'ch2_discovery_stop'
      });
    })
    .catch(errorRouteHandler(next));
});
  ...
  dbService
    ...
    .then(() => {
      dbService.executeQueryByStr(
        'delete from info_event_log_ch2 where channel_number = ?',
        [2]
      );
      dbService.executeQueryByStr(
        'delete from fault_event_log_ch2 where channel_number = ?',
        [2]
      );
      dbService.executeQueryByStr(
        'delete from tripwave_log_ch2 where channel_number = ?',
        [2]
      );
      dbService.executeQueryByStr(
        'delete from remote_smtp where channel_number = ?',
        [2]
      );
      dbService.executeQueryByStr(
        'delete from remote_gcm where channel_number = ?',
        [2]
      );
      dbService.insert('table_change_info', {
        table_name: 'control_discovery',
        column_name: 'ch2_initialize'
      });
      res.send({ success: true });
    })
    .catch(errorRouteHandler(next));
...
router.get('/backupDiscoveredDevices', function (req, res, next) {
  dbService
    .selectQueryById('ch2BackupDiscoveredDevices')
    .then((result) => {
      res.send(result);
    })
    .catch(errorRouteHandler(next));
...
router.get('/discoveredDevices', function (req, res, next) {
  dbService
    .selectQueryById('ch2DiscoveredDevices')
    .then((result) => {
      res.send(result);
    })
    .catch(errorRouteHandler(next));
...
router.post('/hmiEnable', function (req, res, next) {
  dbService
    .select('information_system')
    .then((result) => {
      let statusInfo = result[0];
      if (statusInfo.ch1_status === STATIC_CONFIG.CH_DISCOVERY_STATUS_READY) {
        return Promise.reject(
          new AppError(
            '채널1의 autodiscovery 작업을 실행한 후 hmi 실행이 가능합니다'
          )
        );
      } else {
        return dbService
          ...
          .then((result) => {
            let discoveryInfo = result[0];
            logger.info(
              'check discoveryInfo : ' + JSON.stringify(discoveryInfo)
            );
            let updateControlDiscoveryInfo = {};
...
router.post('/hmiDisable', function (req, res, next) {
  dbService
    .select('information_system')
    .then(() => {
      return dbService
...
// 채널2 hmiRun 수행
router.post('/hmiRun', function (req, res, next) {
  ...
  dbService
    ...
    .then((result) => {
      ...
      if (statusInfo.ch1_status === STATIC_CONFIG.CH_DISCOVERY_STATUS_READY) {
        return Promise.reject(
          new AppError(
            '채널1의 autodiscovery 작업을 실행한 후 hmi run이 가능합니다'
          )
        );
      } else {
        return dbService
