...
// 커스텀 디바이스 수정
router.put('/:channel_number/:physical_identifier', function (req, res, next) {
  dbService
    ...
    .then((result) => {
      ...
      return dbService
        .selectOne(
          'information_discovery_ch' + req.params.channel_number,
          'physical_identifier',
          req.params.physical_identifier
        )
        .then((result) => {
          if (result) {
            ...
            if (result.logical_identifier) {
              ...
              if (
                globalStatusInfo[
                  'ch' + req.params.channel_number + '_status'
                ] === STATIC_CONFIG.CH_DISCOVERY_STATUS_RUNNING
              ) {
                return Promise.reject(
                  new AppError(
                    '채널이 running 중입니다. stop 후 not discovery 장비를 추가해주세요'
                  )
                );
              }
            }
            return dbService
              .selectQueryById(
                'getChannel' + req.params.channel_number + 'MaxLogicalId',
                null
              )
              .then((result) => {
                    ...
                    .then((result) => {
                      if (result) {
                        return dbService.update(
                          'information_device_data_ch' +
                            req.params.channel_number,
                          dataInfo,
                          'logical_identifier',
                          logical_identifier
                        );
                      } else {
                        return dbService.insert(
                          'information_device_data_ch' +
                            req.params.channel_number,
                          dataInfo
                        );
                ...
                promiseList.push(
                  dbService
                    .selectOne(
                      'fast_db_ch' + req.params.channel_number,
                      'logical_identifier',
                      logical_identifier
                    )
                    .then((result) => {
                  ...
                  .then(() => {
                    return dbService.update(
                      'information_discovery_ch' + req.params.channel_number,
                      {
                        ...
                        device_type: req.body.device_name
                      },
                      'physical_identifier',
                      req.params.physical_identifier
                    );
                  ...
                  .then(() => {
                    ...
                    dbService.insert('table_change_info', {
                      table_name:
                        'information_device_data_ch' +
                        req.params.channel_number,
                      column_name: logical_identifier
                    });
                    dbService.insert('table_change_info', {
                      table_name: 'fast_db_ch' + req.params.channel_number,
                      column_name: 'all'
                    });
                    dbService.insert('table_change_info', {
                      table_name:
            ...
            return Promise.reject(
              new AppError(
                '정보가 존재하지 않습니다. autodiscovery를 다시 수행해주세요'
              )
            );
          }
        });
...
router.get('/:channel_number/:physical_identifier', function (req, res, next) {
  dbService
    .selectQueryById('findCustomDeviceChannel' + req.params.channel_number, [
      req.params.physical_identifier
    ])
    .then((result) => {
      res.send(result[0]);
...
// 커스텀 디바이스 정보 삭제하기
router.delete('/:channel_number/:physical_identifier', function (
  req,
  res,
  next
) {
    ...
    .then((result) => {
      if (result.logical_identifier) {
        ...
        return Promise.all(promiseList).then(() => {
          ...
          dbService.insert('table_change_info', {
            table_name: 'information_discovery_ch' + req.params.channel_number,
            column_name: 'all'
          });
        });
      } else {
