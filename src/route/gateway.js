...
// cloud ping
router.post('/cloudPing', function (req, res, next) {
  // validate : 'url'
  ...
  dbService
    ...
    .then((result) => {
      ...
      return dbService
        .executeQueryByStr(
          'delete from setting_info where Category = ? and Type = ?',
          ['CloudInfo', 'Response']
        )
        .then(() => {
          let promiseList = [];
          ...
          promiseList.push(
            dbService.executeQueryById('updateSettingInfo', [
              STATIC_CONFIG.CLOUD_STATUS_ING,
              'CloudInfo',
              'Status'
            ])
          );
...
router.post('/cloudRun', function (req, res, next) {
  dbService
    .executeQueryById('updateSettingInfo', [
      STATIC_CONFIG.CLOUD_STATUS_RUN,
      'CloudInfo',
      'Status'
    ])
    .then(() => {
...
router.post('/cloudStop', function (req, res, next) {
  dbService
    .executeQueryById('updateSettingInfo', [
      STATIC_CONFIG.CLOUD_STATUS_STOP,
      'CloudInfo',
      'Status'
    ])
    .then(() => {
    ...
    .then((result) => {
      ...
      result.forEach((info) => {
        ...
        if (
          ...
          responseResult.status !== STATIC_CONFIG.CLOUD_STATUS_ING
        ) {
...
router.post('/autoDiscoveryStart', function (req, res, next) {
  ...
  dbService
    ...
    .then((result) => {
      ...
      let updateRtuInfo = _.pick(req.body, [
        'eth_response_wait_ms',
        'eth_retry_count',
        'eth_start_address',
        'eth_end_address'
      ]);
      ...
      promiseList.push(
        dbService.updateAll('setting_modbus_rtu', updateRtuInfo)
      );
      return Promise.all(promiseList);
    })
    .then(() => {
      res.send({ success: true });
      dbService
        .insert('table_change_info', {
          table_name: 'setting_modbus_rtu',
          column_name: 'all'
        })
        .then(() => {
          dbService.insert('table_change_info', {
            table_name: 'control_discovery',
            column_name: 'eth_discovery_start'
          });
        });
    })
    .catch(errorRouteHandler(next));
...
router.post('/autoDiscoveryStop', function (req, res, next) {
  dbService
    ...
    .then((result) => {
      ...
      return dbService.updateAll(
        'control_discovery',
        updateControlDiscoveryInfo
      );
    })
    .then(() => {
      res.send({ success: true });
      dbService.insert('table_change_info', {
        table_name: 'control_discovery',
        column_name: 'eth_discovery_stop'
      });
    })
    .catch(errorRouteHandler(next));
});
...
router.get('/backupDiscoveredGateways', function (req, res, next) {
  dbService
    .selectQueryById('backupDiscoveredGateways')
    .then((result) => {
      res.send(result);
    })
    .catch(errorRouteHandler(next));
...
router.get('/discoveredGateways', function (req, res, next) {
  dbService
    .selectQueryById('discoveredGateways')
    .then((result) => {
      res.send(result);
    })
    .catch(errorRouteHandler(next));
...
router.get('/basic', function (req, res, next) {
  dbService
    ...
    .then((result) => {
      ...
      gatewayInfo = _.pick(gatewayInfo, [
        'product_code',
        'product_name',
        'serial_number',
        'manufacturer_name',
...
router.put('/basic', function (req, res, next) {
  let updateGatewayInfo = _.pick(req.body, [
    'product_code',
    'product_name',
    'serial_number',
    'manufacturer_name',
  ...
  dbService
    .updateAll('gateway_data', updateGatewayInfo)
    .then(() => {
      res.send({ success: true });
      dbService.insert('table_change_info', {
...
router.get('/systemLogs', function (req, res, next) {
  dbService
    .selectQueryByStr('select * from system_log order by id desc')
    .then((result) => {
      res.send(result);
    })
    .catch(errorRouteHandler(next));
...
router.get('/faultEventLogs', function (req, res, next) {
  ...
  const page = req.query.page ? Number(req.query.page) : 1;
  dbService
    .selectQueryById('getFaultEventAllCount')
    .then((totalCountQueryResult) => {
      const totalCount = totalCountQueryResult[0].totalCount;
      let result = {};
      result.totalCount = totalCount;
      return dbService
        .selectQueryById('getFaultEventAllList', [
          pageSize * (page - 1),
          pageSize
        ])
        .then((data) => {
...
router.get('/tripWaveLogs', function (req, res, next) {
  ...
  const page = req.query.page ? Number(req.query.page) : 1;
  dbService
    .selectQueryById('getTripwaveAllCount')
    .then((totalCountQueryResult) => {
      const totalCount = totalCountQueryResult[0].totalCount;
      let result = {};
      result.totalCount = totalCount;
      return dbService
        .selectQueryById('getTripwaveAllList', [
          pageSize * (page - 1),
          pageSize
        ])
        .then((data) => {
