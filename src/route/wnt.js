...
function read_resolv_conf() {
    var child = spawnSync('/bin/sed', ['-ne', 's/^nameserver *\\([1-9][0-9]*\\.[0-9]*\\.[0-9]*\\.[0-9]*\\).*$/\\1/p', '/etc/resolv.conf']);
    ...
    fs.readFile(WNTSYSCONF_PATH, 'utf8', (err, data) => {
        ...
        try {
            ...
            if (!wntsysconf.network.configured) {
                ...
                try {
                    fs.writeFileSync(WNTSYSCONF_PATH, JSON.stringify(wntsysconf, undefined, 2));
                } catch (e) {
                    throw e;
                }
            }
    ...
    timedatectl.on('close', function (code) {
        if (code == 0) {
            ...
            while ((tz = tzlist.shift())) {
                if (tz.length > 0) {
                    if (tz === 'Asia/Seoul')
                        tzs.push({ 'value': 'ROK', 'text': tz });
                    else
                        tzs.push({ 'value': tz, 'text': tz });
                }
            }
...
router.post('/setWan', function (req, res) {
    fs.readFile(WNTSYSCONF_PATH, 'utf8', (err, data) => {
        if (err)
            throw new AppError('file not found', [err], 404);
        ...
        try {
            ...
            if (changed) {
                ...
                if (isWlWan && req.body.netmode !== 'nat') {
                    ...
                    throw new AppError('Bridge node impossible when wifi mode is station.', [e], 403);
                }
                var wan = fs.createWriteStream('/etc/systemd/network/' + (isWlWan?'30-wlan.network':'br-wan.network'));
                wan.write('[Match]\n');
                ...
                if (req.body.wan.proto === 'dhcp') {
                    ...
                    wan.write('Address=' + req.body.wan.ipaddr + '/' + req.body.wan.netmask + '\n');
                ...
                if (wntsysconf.network.netmode != req.body.netmode) {
                    var eth0 = fs.createWriteStream('/etc/systemd/network/10-eth.network');
                    eth0.write('[Match]\n');
                    eth0.write('Name=eth0\n\n');
                    eth0.write('[Network]\n');
                    if (req.body.netmode === 'nat')
                        eth0.write('Bridge=br-lan\n');
                    else
                        eth0.write('Bridge=br-wan\n');
                    eth0.end();

                    var eth1 = fs.createWriteStream('/etc/systemd/network/10-eth1.network');
                    eth1.write('[Match]\n');
                ...
                var child = exec('/bin/systemctl restart systemd-networkd.service', function (error, stdout, stderr) {
...
router.post('/setLan', function (req, res) {
    fs.readFile(WNTSYSCONF_PATH, 'utf8', (err, data) => {
        ...
        try {
            ...
            if (wntsysconf.network.lan.ipaddr != req.body.lan.ipaddr ||
            ...
            if (changed) {
                ...
                brlan.write('[Network]\n');
                brlan.write('Address=' + req.body.lan.ipaddr + '/' + req.body.lan.netmask + '\n');
                brlan.end();
                ...
                if (req.body.dhcpd.active) {
                    ...
                    dhcpd_conf.write('opt subnet ' + netmask2subnet(req.body.lan.netmask) + '\n');
                    dhcpd_conf.write('opt router ' + req.body.lan.ipaddr + '\n');
                    if (dns.length > 0)
                        dhcpd_conf.write('opt dns ' + dns.join(' ') + '\n');
                    else
                        dhcpd_conf.write('opt dns ' + wntsysconf.network.wan.dns.primary + ' ' + wntsysconf.network.wan.dns.secondary + '\n');
                    dhcpd_conf.end();
                }
...
router.post('/setWlan', function (req, res) {
    fs.readFile(WNTSYSCONF_PATH, 'utf8', (err, data) => {
        ...
        try {
            ...
            if (req.body.wlan.mode === 'station' && wntsysconf.network.netmode !== 'nat') {
                console.log('netmode=' + wntsysconf.network.netmode + ', wifimode=station');
                throw new AppError('Bridge mode impossible when wifi mode is station.', [e], 403);
            }
            ...
            if (wntsysconf.network.wlan.mode != req.body.wlan.mode ||
            ...
            if (wntsysconf.network.wlan.mode === 'disabled')
                wasDisabled = true;
            ...
            if (changed) {
                if (req.body.wlan.mode === 'ap') {
                    ...
                    if (req.body.wlan.security)
                        args.push(req.body.wlan.passphrase);
                    ...
                    if (wntsysconf.network.netmode !== 'nat') {
                        console.log('netmode=' + wntsysconf.network.netmode + ', wifimode=station');
                        throw new AppError('Station mode impossible when netmode is not NAT.', [e], 403);
                    }
                ...
                if (wntsysconf.network.wan.proto === 'dhcp') {
                    ...
                    if (wntsysconf.network.wan.dns.mode !== 'auto') {
                        if (wntsysconf.network.wan.dns.primary && wntsysconf.network.wan.dns.primary.length)
                            dat += 'DNS=' + wntsysconf.network.wan.dns.primary + '\n';
                        if (wntsysconf.network.wan.dns.secondary && wntsysconf.network.wan.dns.secondary.length)
                            dat += 'DNS=' + wntsysconf.network.wan.dns.secondary + '\n';
                    }
                }
                else {
                    dat += 'Address=' + wntsysconf.network.wan.ipaddr + '/' + wntsysconf.network.wan.netmask + '\n';
                ...
                if (isWlWan)
                    execSync('/bin/sed -i -e \'s/^ExecStart=.* br-wan .*/ExecStart=\\/usr\\/sbin\\/iptables -t nat -A POSTROUTING -o wlan0 -j MASQUERADE/\' /lib/systemd/system/nat.service');
                else
                    execSync('/bin/sed -i -e \'s/^ExecStart=.* wlan0 .*/ExecStart=\\/usr\\/sbin\\/iptables -t nat -A POSTROUTING -o br-wan -j MASQUERADE/\' /lib/systemd/system/nat.service');
    ...
    fs.readFile(WNTSYSCONF_PATH, 'utf8', (err, data) => {
        ...
        try {
            ...
            if (wntsysconf.system.timesyncd.timezone != req.body.timesyncd.timezone) {
                var child = execSync('/usr/bin/timedatectl set-timezone ' + req.body.timesyncd.timezone, function (error, stdout, stderr) {
                    console.log('stdout: ' + stdout);
                    console.log('stderr: ' + stderr);
                    if (error !== null) {
                        console.log('exec error: ' + error);
            ...
            if (req.body.timesyncd.enabled == false) {
                var dateString = req.body.timesyncd.manual_date.year + '-'
                ...
                if (wntsysconf.system.timesyncd.enabled) {
                    ...
                    var child = execSync('/bin/systemctl disable systemd-timesyncd.service');
                ...
                if (wntsysconf.system.timesyncd.ntpservers.toString() != req.body.timesyncd.ntpservers.toString()) {
                    var ws = fs.createWriteStream('/etc/systemd/timesyncd.conf');
                    ws.write('[Time]\nNTP=' + req.body.timesyncd.ntpservers.join(' ') + '\n');
                    ws.end();

                    timesyncd_action = 'restart';
                    wntsysconf.system.timesyncd.ntpservers = req.body.timesyncd.ntpservers;
                    changed = true;
                }

                if (wntsysconf.system.timesyncd.enabled == false) {
                    var child = execSync('/bin/systemctl enable systemd-timesyncd.service');
            ...
            if (changed) {
                ...
                var child = exec('/bin/systemctl ' + timesyncd_action + ' systemd-timesyncd.service', function (error, stdout, stderr) {
                    console.log('stdout: ' + stdout);
                    console.log('stderr: ' + stderr);
                    if (error !== null) {
                        console.log('exec error: ' + error);
...
function get_standby_part()
{
    ...
    var active_part = child.stdout.toString().split('\n').filter((bootenv) => {
...
function remove_old_kmodules(standby_part)
{
    fs.readdirSync('/lib/modules/').forEach( (file, index) => {
        if (file !== fwupProgress.curKernVer.uts_rel && file !== fwupProgress.newKernVer.uts_rel) {
...
function install_kmodules(standby_part)
{
    ...
    var files = child.stdout.toString().split('\n').filter((file) => {
    ...
    var extracted = 0;
    var remained = '';
    child = spawn('/bin/tar', ['xzvf', '/tmp/fwup/kernelmodules.tar.gz', '-C', '/lib/modules/']);
    child.stdout.on('data', (data) => {
        ...
        console.log('[fwup]: kernel module progress: ' + extracted + '/' + file_cnt + '(' + fwupProgress.progressMOD + '%)');

        if (fwupProgress.progressMOD > 100)
    ...
    child.on('close', (code) => {
        if (code) {
            ...
            exec('/sbin/fw_setenv active_part ' + standby_part.part, (error, stdout, stderr) => {
                if (error) {
...
function flash_kernel(standby_part)
{
    ...
    var remained = '';
    var child = spawn('/usr/sbin/nandwrite', ['-b', '1', '-n', '-p', standby_part.kern_dev, '/tmp/fwup/zImage.bin']);
    child.stdout.on('data', (data) => {
        var isPartial = false;
        var dataStr = remained + data.toString();
        if (dataStr[dataStr.length - 1] != '\n') 
        ...
        if (lines.length > 0) {
            ...
            if ((last_offset + NAND_BLOCK_SIZE) > img_size)
                ...
                fwupProgress.progressKERN = Math.round((last_offset + NAND_BLOCK_SIZE)*100/img_size, 0);
...
{
    var child = spawn('/usr/sbin/nandwrite', ['-b', '1', '-n', '-p', standby_part.dtb_dev, '/tmp/fwup/zImage.dtb']);
    child.on('close', (code) => {
        if (code) {
...
function start_kernel_installation() /* === start to install dtb */
{
...
{
    mkdirp('/tmp/fwup');
    var child = spawn('/bin/tar', ['-xf', FWUP_DIR + '/' + FWUP_TAR, '-C', '/tmp/fwup/']);
    child.on('close', (code)=> {
...
router.post('/fwup', upload_kernel.single('kernel_img'), function (req, res) {
    if (fwupProgress.progressEnd === 'progressing') {
        ...
        fs.rename(FWUP_DIR + '/' + req.file.filename, FWUP_DIR + '/' + FWUP_TAR, (err) => {
            if (err) {
                console.error('[fwup]: failed to rename ' + FWUP_DIR + '/' + req.file.filename + ' to ' + FWUP_DIR + '/' + FWUP_TAR);
                fwupProgress.progressEnd = 'failure';
        ...
        while (lines.length > 0) {
            ...
            if (line_last == '>') {
                ...
                while (line_last.length > 0) {
                    if (line_last.startsWith('bssid') || line_last.indexOf('scan_result') >= 0) {
...
router.post('/scanStart', (req, res) => {
    try {
        if (scan_ap.status === 'done' || scan_ap.status === 'timeout' || scan_ap.status === 'failure' || scan_ap.status == 'none') {
            doScanAP();
        }
        res.send(scan_ap);
    } catch (e) {
