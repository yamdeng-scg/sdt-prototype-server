...
if (DEBUG_MODE) {
  channelTestDataList = [
    {
      device_type: 'ACBOCR-A',
      manufacturer_name: 'LS산전연구실',
      hw_version: '0.1',
      sw_version: '0.2',
      serial_number: '123-456',
      product_code: 'ACBOCR-A',
      name: 'ACBOCR-A'
    },
    {
      device_type: 'ACBOCR-S',
      manufacturer_name: 'LS산전연구실',
      hw_version: '0.1',
      sw_version: '0.2',
      serial_number: '123-456',
      product_code: 'ACBOCR-S',
      name: 'ACBOCR-S'
    },
    {
      device_type: 'ACBOCR-P',
      manufacturer_name: 'LS산전연구실',
      hw_version: '0.1',
      sw_version: '0.2',
      serial_number: '123-456',
      product_code: 'ACBOCR-P',
      name: 'ACBOCR-P'
    },
    {
      device_type: 'TRIO',
    ...
    {
      device_type: 'SMARTMCCB',
      manufacturer_name: 'LS산전연구실',
      hw_version: '0.1',
      sw_version: '0.2',
      serial_number: '123-456',
      product_code: 'SMARTMCCB',
      name: 'SMARTMCCB'
    },
    {
      device_type: 'MLINK',
    ...
    {
      device_type: 'ENERGYMETER_2P',
      manufacturer_name: 'LS산전연구실',
      hw_version: '0.1',
      sw_version: '0.2',
      serial_number: '123-456',
      product_code: 'ENERGYMETER_2P',
      name: 'ENERGYMETER_2P'
    },
    {
      device_type: 'ENERGYMETER_3P',
      manufacturer_name: 'LS산전연구실',
      hw_version: '0.1',
      sw_version: '0.2',
      serial_number: '123-456',
      product_code: 'ENERGYMETER_3P',
      name: 'ENERGYMETER_3P'
    },
    {
      device_type: 'POWERTAG',
      manufacturer_name: 'LS산전연구실',
      hw_version: '0.1',
      sw_version: '0.2',
      serial_number: '123-456',
    ...
    {
      device_type: 'GIMACB_B',
      manufacturer_name: 'LS산전연구실',
      hw_version: '0.1',
      sw_version: '0.2',
      serial_number: '123-456',
      product_code: 'GIMACB_B',
      name: 'GIMACB_B'
    },
    {
      device_type: 'GIMACB_T',
      manufacturer_name: 'LS산전연구실',
      hw_version: '0.1',
      sw_version: '0.2',
      serial_number: '123-456',
      product_code: 'GIMACB_T',
      name: 'GIMACB_T'
    },
    { device_type: 'noname' }
  ];
...
function executeAutoDiscoveryConfirm(channelNumber) {
  ...
  for (let index = 0; index < channelTestDataList.length; index++) {
    ...
    let discoveryChInfo = {
      ...
      logical_identifier:
        channelTestDataList[index].device_type === 'noname' ? null : identifier,
      status:
        channelTestDataList[index].device_type === 'noname'
          ? STATIC_CONFIG.DEVICE_STATUS_NOT_DISCOVERY
          : STATIC_CONFIG.DEVICE_STATUS_DISCOVERED,
      device_type: channelTestDataList[index].device_type
    };
    insertPromiseList.push(
      dbService.insert(
        'information_discovery_ch' + channelNumber,
        discoveryChInfo
      )
    );
    ...
    insertPromiseList.push(
      dbService.insert(
        'information_device_data_ch' + channelNumber,
        discoveryDataInfo
      )
    );
    let fastDbChInfo = {
    ...
    insertPromiseList.push(
      dbService.insert('fast_db_ch' + channelNumber, fastDbChInfo)
    );
    Promise.all(insertPromiseList).then(() => {
      ...
      updateStatusInfo['ch' + channelNumber + '_status'] =
        STATIC_CONFIG.CH_DISCOVERY_STATUS_STOP;
...
function executeAutoDiscovery(channelNumber) {
  ...
  deletePromiseList.push(
    dbService.deleteAll('information_device_data_ch' + channelNumber)
  );
  ...
  Promise.all(deletePromiseList).then(() => {
    ...
    for (let index = 0; index < channelTestDataList.length; index++) {
      ...
      insertPromiseList.push(
        dbService.insert(
          'information_discovery_ch' + channelNumber + '_bak',
          discoveryChInfo
        )
      );
      if (channelTestDataList[index].device_type === 'noname') {
    ...
    Promise.all(insertPromiseList).then(() => {
      ...
      let updateStatusInfo = {};
      updateStatusInfo['ch' + channelNumber + '_status'] =
        STATIC_CONFIG.CH_DISCOVERY_STATUS_DISCOVERED;
      dbService.updateAll('information_system', updateStatusInfo);
    });
  });
}
...
function executeRun(channelNumber) {
  ...
  Promise.all(promiseList).then(() => {
    setTimeout(() => {
      let updateStatusInfo = {};
      updateStatusInfo['ch' + channelNumber + '_status'] =
        STATIC_CONFIG.CH_DISCOVERY_STATUS_RUNNING;
      dbService.updateAll('information_system', updateStatusInfo);
...
function executeInit(channelNumber) {
  ...
  Promise.all(deletePromiseList).then(() => {
    ...
    let updateStatusInfo = {};
    updateStatusInfo['ch' + channelNumber + '_status'] =
      STATIC_CONFIG.CH_DISCOVERY_STATUS_READY;
    dbService.updateAll('information_system', updateStatusInfo);
  });
}
...
function executeAutoDiscoveryConfirmByGateway() {
  ...
  Promise.all(deletePromiseList).then(() => {
    ...
    for (let index = 0; index < gatewayTestDataList.length; index++) {
      ...
      insertPromiseList.push(
        dbService.insert('information_discovery_eth', discoveryGatewayInfo)
      );
    }
    Promise.all(insertPromiseList).then(() => {
      let updateControlDiscoveryInfo = {};
...
function executeAutoDiscoveryByGateway() {
  ...
  Promise.all(deletePromiseList).then(() => {
    ...
    for (let index = 0; index < gatewayTestDataList.length; index++) {
      ...
      insertPromiseList.push(
        dbService.insert('information_discovery_eth_bak', discoveryGatewayInfo)
      );
    }
    Promise.all(insertPromiseList).then(() => {
      let updateControlDiscoveryInfo = {};
...
function executeRunByGateway() {
  ...
  Promise.all(promiseList).then(() => {
    setTimeout(() => {
      ...
      dbService.updateAll(
        'information_discovery_eth',
        updateDiscoveryGatewayInfo
      );
...
service.runCheckAutoDiscoveryTask = function () {
  checkAutoDiscoveryTaskId = setInterval(() => {
      ...
      dbService.select('control_discovery').then((result) => {
        if (result && result.length === 1) {
          ...
          if (discoveryControlInfo.ch1_discovery_start) {
            ...
            statusInfo.ch1_status >
              STATIC_CONFIG.CH_DISCOVERY_STATUS_DISCOVERING &&
            discoveryControlInfo.ch1_run
          ) {
            executeRun(1);
          } else if (
            statusInfo.ch1_status >
          ...
          if (discoveryControlInfo.ch2_discovery_start) {
            ...
            discoveryControlInfo.ch2_initialize ||
            discoveryControlInfo.ch2_hmi
          ) {
            executeInit(2);
          } else if (
            statusInfo.ch2_status >= STATIC_CONFIG.CH_DISCOVERY_STATUS_STOP &&
            discoveryControlInfo.ch2_run
          ) {
            executeRun(2);
          } else if (
            statusInfo.ch2_status >= STATIC_CONFIG.CH_DISCOVERY_STATUS_STOP &&
          ...
          if (discoveryControlInfo.eth_discovery_start) {
            ...
            statusInfo.eth_status >= STATIC_CONFIG.CH_DISCOVERY_STATUS_STOP &&
            discoveryControlInfo.eth_run
          ) {
            executeRunByGateway();
          } else if (
            statusInfo.eth_status >= STATIC_CONFIG.CH_DISCOVERY_STATUS_STOP &&
    ...
    dbService.select('setting_info').then((result) => {
      // 1인 경우 작업하고 2로 바꾸고 response를 insert 한다
      result.forEach((info) => {
        if (
          info.Category === 'CloudInfo' &&
          info.Type === 'Status' &&
          info.Value === '1'
        ) {
          ...
          dbService.executeQueryById('updateSettingInfo', [
            '2',
            'CloudInfo',
            'Status'
          ]);
...
service.runSystemLogDeleteTask = function () {
  systemLogDeleteTaskId = setInterval(() => {
    dbService
      .selectQueryByStr(
        'select ifnull(count(id), 0) as totalCount FROM system_log'
      )
      .then((result) => {
        ...
        if (totalCount > CONFIG.systemLogTableMax) {
          dbService
            .selectQueryByStr('select max(id) as maxId from system_log')
            .then((result) => {
              const deleteRowId = result[0].maxId - CONFIG.systemLogTableMax;
              logger.info('runSystemLogTask deleteRowId : ' + deleteRowId);
              dbService.executeQueryByStr(
                'delete from system_log where id < ?',
                deleteRowId
              );
            });
        }
...
service.runGetLogInfoByClientServer = function () {
  getLogInfoTaskId = setInterval(() => {
    // device 전체 목록 가져오기
    dbService
      .selectQueryById('discoveredDevicesByAllChannel')
      .then((result) => {
        if (result && result.length > 0) {
          result.forEach((findDevice) => {
            let systemEventHttpUrl =
              CONFIG.clientServer.url +
              '/' +
              findDevice.logical_identifier +
              '/systemEvent';
            let faultEventHttpUrl =
              CONFIG.clientServer.url +
              '/' +
              findDevice.logical_identifier +
              '/faultEvent';
            ...
            if (checkEventLogByDeviceType(findDevice.device_type)) {
              request
                  ...
                  if (response.body && response.body.length) {
                    response.body.forEach((logData) => {
                      ...
                      if (findDevice.channel_number === 1) {
                        ...
                        dbService
                          .selectQueryById('ch2InfoEventDuplCheck', [
                            findDevice.logical_identifier,
                            logData.logDate
                          ])
                          .then((result) => {
                ...
                .catch((err) => {
                  logger.error(
                    'runGetLogInfoByClientServer systemEventHttpUrl http call error : ' +
                      err
                  );
                });
              request
                .get(faultEventHttpUrl)
                .then((response) => {
                  if (response.body && response.body.length) {
                    response.body.forEach((logData) => {
                      logData.logical_identifier =
                      ...
                      if (findDevice.channel_number === 1) {
                        dbService
                          .selectQueryById('ch1FaultEventDuplCheck', [
                            findDevice.logical_identifier,
                            logData.logDate
                          ])
                          .then((result) => {
                        ...
                        dbService
                          .selectQueryById('ch2FaultEventDuplCheck', [
                            findDevice.logical_identifier,
                            logData.logDate
                          ])
                          .then((result) => {
                ...
                .catch((err) => {
                  logger.error(
                    'runGetLogInfoByClientServer faultEventHttpUrl http call error : ' +
                      err
                  );
                });
            }
              ...
              request
                ...
                .then((response) => {
                  if (response.body) {
                    ...
                    if (findDevice.channel_number === 1) {
                      dbService
                        .selectQueryById('ch1TripwaveDuplCheck', [
                          findDevice.logical_identifier,
                          logData.logDate
                        ])
                        .then((result) => {
                      ...
                      dbService
                        .selectQueryById('ch2TripwaveDuplCheck', [
                          findDevice.logical_identifier,
                          logData.logDate
                        ])
                        .then((result) => {
                ...
                .catch((err) => {
                  logger.error(
                    'runGetLogInfoByClientServer tripwaveHttpUrl http call error : ' +
                      err
                  );
                });
            }
      ...
      .then((result) => {
        if (result && result.length > 0) {
          result.forEach((info) => {
            ...
            dbService
              .selectQueryByStr(
                'select ifnull(count(id), 0) as totalCount FROM ' +
                  infoEventTableName +
                  ' where logical_identifier = ?',
                [info.logical_identifier]
              )
              .then((result) => {
                const totalCount = result[0].totalCount;
                logger.info(
                  'runDeviceLogTask info_event_log[' +
                    info.logical_identifier +
                    '] totalCount : ' +
                    totalCount
                );
                if (totalCount > CONFIG.monitoringLogTableMax) {
                  dbService
                    .selectQueryByStr(
                      'select ifnull(min(t.id), 0) as minId from (select id from ' +
                        infoEventTableName +
                        ' where logical_identifier = ? order by id desc limit 0, ?) t',
                      [info.logical_identifier, CONFIG.monitoringLogTableMax]
                    )
                    .then((result) => {
                      if (result && result.length && result[0].minId) {
                        ...
                        dbService.executeQueryByStr(
                          'delete from ' +
                            infoEventTableName +
                            ' where logical_identifier = ? and id < ?',
                          [info.logical_identifier, deleteRowId]
                        );
                      }
                    });
                }
            ...
            // 실패 이벤트 로그 삭제
            let faultEventTableName =
              'fault_event_log_ch' + info.channel_number;
            dbService
              .selectQueryByStr(
                'select ifnull(count(id), 0) as totalCount FROM ' +
                  faultEventTableName +
              ...
              .then((result) => {
                const totalCount = result[0].totalCount;
                logger.info(
                  'runDeviceLogTask fault_event_log[' +
                    info.logical_identifier +
                    '] totalCount : ' +
                    totalCount
                );
